...
class AmazonSpider(scrapy.Spider):
    ...
    start_urls = [
        "https://www.amazon.in/s?k="+(key_word)+"&ref=nb_sb_noss_2"
        ]
    numbers=0
    def parse(self,response):
        for sel in response.css(".s-widget-spacing-small"):
            item = AmazonScrapyItem()
            title = sel.xpath(".//h2[@class='a-size-mini a-spacing-none a-color-base s-line-clamp-2']//span//text()").get()
            if title is None:
                title=sel.xpath('.//*[@class="a-size-medium a-color-base a-text-normal"]//text()').extract_first()
            item['title']=title
